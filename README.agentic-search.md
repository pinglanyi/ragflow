...
| 工具 | JSON 请求体 | `data` |
|---|---|---|
| `search` | `query` 必填；`top_k` 默认 5；`exclude_ids` 可选 chunk ID 数组；`dataset_selection_mode` 为 `all`（默认）或 `auto`；`dataset_names` 可选逗号分隔的知识库名称或 ID；旧 `dataset_ids` 仍可用 | `chunks`、`selected_datasets`、`dataset_selection_mode`、`search_metadata`；返回模式可为 `all`、`auto`、`manual` |
...
需要保留原来的描述路由时，传 `"dataset_selection_mode":"auto"` 且不传知识库范围；
后端会根据查询、知识库名称和描述选择 1–3 个 embedding 兼容的库。`auto` 与显式
`dataset_names`/`dataset_ids` 不能同时使用。
...
curl -sS http://127.0.0.1:9380/api/v1/agentic-search/tools/search \
  ...
  -d '{"query":"CAN 扩展模块接线和接地有什么要求？","top_k":5,"dataset_names":"产品库"}' | jq .
...
## 文件候选检索 API（与 chunk retrieval 同级）
...
curl -sS http://127.0.0.1:9380/api/v1/retrieval-doc-name \
  -H "Authorization: Bearer $RAGFLOW_API_KEY" -H 'Content-Type: application/json' \
  -d '{"keyword":"E502","mode":"phrase","scan_meta":false,"top_k":5,"dataset_names":"产品库"}' | jq .

# 仅通过 meta_fields.description 命中：文件名可以完全不含该查询词
curl -sS http://127.0.0.1:9380/api/v1/retrieval-doc-name \
  -H "Authorization: Bearer $RAGFLOW_API_KEY" -H 'Content-Type: application/json' \
  -d '{"keyword":"温度 采集","mode":"term","scan_meta":true,"top_k":5,"dataset_names":"产品库"}' | jq .
```
...
- [七个原生工具 API（9380）](#七个原生工具-api9380)
- [文件候选检索 API（与 chunk retrieval 同级）](#文件候选检索-api与-chunk-retrieval-同级)

...
start_taskexec() {
    if [ -f "$PID_TASKEXEC" ] && kill -0 "$(cat "$PID_TASKEXEC")" 2>/dev/null; then
        echo "[taskexec] 已在运行 PID=$(cat "$PID_TASKEXEC")"
        return
    fi
    echo "[taskexec] 启动 Task Executor..."
    cd "$PROJECT_DIR"
    nohup "$VENV_PYTHON" rag/svr/task_executor.py 0 > "$LOG_TASKEXEC" 2>&1 &
    echo $! > "$PID_TASKEXEC"
    sleep 3
    if kill -0 "$(cat "$PID_TASKEXEC")" 2>/dev/null; then
        echo "[taskexec] 启动成功 PID=$(cat "$PID_TASKEXEC")"
    else
        echo "[taskexec] 启动失败, 查看 $LOG_TASKEXEC"
        return 1
    fi
}
...
stop_all() {
    for pid_file in "$PID_SERVER" "$PID_TASKEXEC" "$PID_WEB"; do
        if [ -f "$pid_file" ]; then
            pid=$(cat "$pid_file")
...
status_all() {
    ...
    echo " RAGFlow 服务状态"
    echo "══════════════════════════════════════"
    for name in server taskexec web; do
        pid_var="PID_$(echo "$name" | tr '[:lower:]' '[:upper:]')"
